    ...
    <div className="space-y-2">
      <div className="relative">
        ...
        {!value ? (
          <div
            className={`
              relative w-full p-4 sm:p-8 border-2 border-dashed rounded-xl text-center cursor-pointer
          ...
          >
            {isProcessing && (
              <div className="absolute inset-0 bg-white bg-opacity-75 rounded-xl flex items-center justify-center z-10">
                <div className="flex flex-col items-center space-y-2">
                  ...
                  <p className="text-xs sm:text-sm text-gray-600">Processando arquivo...</p>
                </div>
              </div>
            )}
            ...
            <div className="flex flex-col items-center space-y-3 sm:space-y-4">
              {icon ? (
                <div className="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-r from-blue-500 to-purple-600 rounded-xl flex items-center justify-center text-white group-hover:scale-110 transition-transform duration-300">
                  {icon}
                </div>
              ) : (
                <div className="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-r from-blue-500 to-purple-600 rounded-xl flex items-center justify-center text-white group-hover:scale-110 transition-transform duration-300">
              ...
              <div className="flex items-center space-x-2 text-xs sm:text-sm text-gray-400">
                ...
                <span className="hidden sm:inline">Formatos aceitos: {accept}</span>
                <span className="sm:hidden">Formatos: {accept}</span>
              </div>
            </div>
          </div>
        ) : (
          <div className="w-full p-4 sm:p-6 bg-green-50 border-2 border-green-200 rounded-xl animate-success-check">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3 sm:space-x-4 min-w-0 flex-1">
                {getFileIcon(value.name)}
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-gray-900 truncate text-sm sm:text-base">{value.name}</p>
                  <p className="text-xs sm:text-sm text-gray-500">{formatFileSize(value.size)}</p>
                </div>
              </div>
              ...
              <button
                ...
                className="p-1.5 sm:p-2 text-red-500 hover:bg-red-100 rounded-lg transition-colors duration-200 hover:scale-110 flex-shrink-0"
      ...
      {description && !error && !value && (
        <p className="text-xs sm:text-sm text-gray-500 flex items-start">
